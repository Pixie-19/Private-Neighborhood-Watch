/**
 * Standalone deploy script for the Anon Incident Report (neighbour) contract.
 *
 * Usage:
 *   npm run build && npm run deploy
 *
 * Prompts for a hex seed (or generates a new one), builds a wallet,
 * waits for sync + funds + dust, deploys the neighbour contract,
...
import { Contract, ledger as neighbourLedger } from './managed/neighbour/contract/index.js';
...
const zkConfigPath = path.resolve(currentDir, '..', 'src', 'managed', 'neighbour');
...
async function main() {
  ...
  console.log('\n╔══════════════════════════════════════════════════════════╗');
  console.log('║      Anon Incident Report Contract Deploy Script        ║');
  ...
  const zkConfigProvider = new NodeZkConfigProvider<NeighbourCircuits>(zkConfigPath);

  const providers = {
    privateStateProvider: levelPrivateStateProvider<typeof NeighbourPrivateStateId>({
      privateStateStoreName: 'neighbour-private-state',
  ...
  // --- Compile contract ---
  const neighbourCompiledContract = CompiledContract.make('neighbour', Contract).pipe(
    CompiledContract.withVacantWitnesses,
    CompiledContract.withCompiledFileAssets(zkConfigPath),
  );

  // --- Deploy ---
  const contract = await withStatus('Deploying neighbour contract (this may take a few minutes)', async () => {
